...
Scripts operacionais são separados em `scripts/bootstrap`, `scripts/database`, `scripts/imports`, `scripts/quality`, `scripts/security`, `scripts/shared` e `scripts/testing`. O schema tem uma única fonte de verdade em `supabase/migrations/`. A carga inicial consolidada do Diagnóstico de Integridade 2026 fica em `data/bootstrap-2026/`, separada do schema e sem carregar o histórico técnico do banco anterior.
...
`package.json`). Preencha `.env.local` a partir de `.env.example`. O arquivo de exemplo
...
Há **14 migrations timestampadas** em `supabase/migrations/`: 10 migrations imutáveis da baseline greenfield, já validadas em PostgreSQL 17 real, mais evoluções pós-baseline de alteração de prazo, FAMI preliminar, campos de exportação do plano de ação e Data API somente leitura de relatórios oficiais — ver
[`docs/current/BANCO.md`](docs/current/BANCO.md) e
[`docs/current/BASELINE_PRIMEIRA_IMPLANTACAO.md`](docs/current/BASELINE_PRIMEIRA_IMPLANTACAO.md). A validação da baseline em PostgreSQL real está documentada em [`docs/archive/2026-08/POSTGRESQL_REAL_STAGE_5B_RESULT.md`](docs/archive/2026-08/POSTGRESQL_REAL_STAGE_5B_RESULT.md).
...
# O catálogo/formulário deve ser criado e publicado pelo fluxo administrativo.
# O fixture SQL de diagnóstico existente em supabase/testing/fixtures é exclusivo de testes.
...
- [`docs/archive/2026-08/CORRECOES_SEGURANCA_2026-08-02.md`](docs/archive/2026-08/CORRECOES_SEGURANCA_2026-08-02.md): registro técnico do endurecimento de segurança aplicado.
...
- [`docs/archive/AUDITORIA_PROFUNDA_FLUXOS_PERFIS_2026-07-16.md`](docs/archive/AUDITORIA_PROFUNDA_FLUXOS_PERFIS_2026-07-16.md):
  auditoria final de navegação, responsabilidades, transições e contratos entre administrador e respondente.
...
identidade do script administrativo CBM/RN. Consulte
`docs/archive/2026-08/CLEAN_CODE_STAGE_3_RESULT.md`.

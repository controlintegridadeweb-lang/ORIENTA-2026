...
assert(tableOwners.size === 57, `Esperadas 57 tabelas públicas finais; encontradas ${tableOwners.size}.`);
assert(functionOwners.size === 190, `Esperadas 190 funções de aplicação conhecidas após as evoluções atuais; encontradas ${functionOwners.size}.`);
assert(viewOwners.size === 6, `Esperadas 6 views públicas finais; encontradas ${viewOwners.size}.`);
assert(triggerOwners.size === 91, `Esperados 91 triggers finais; encontrados ${triggerOwners.size}.`);

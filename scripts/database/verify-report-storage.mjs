...
async function withReplicaClient(fn) {
  ...
  try {
    ...
    try { await db.query("rollback"); } catch { /* conexão já encerrada */ }
    throw error;
  } finally {
    await db.end();
  }
...
async function cleanupFixture() {
  await withReplicaClient(async (db) => {
    if (reportId) await db.query(`delete from public.reports where id = $1`, [reportId]);
    await db.query(`delete from public.cycle_processings where id = $1`, [ids.processing]);
    await db.query(`delete from public.cycles where id = $1`, [ids.cycle]);
  });

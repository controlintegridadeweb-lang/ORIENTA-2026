...
1. Conta na [Vercel](https://vercel.com) com acesso ao GitHub
   `controlintegridadeweb-lang/ORIENTA-REVISAO`.
...
No projeto Vercel → **Settings → Environment Variables**, cadastre:
...
Marque `SUPABASE_SERVICE_ROLE_KEY` e `CRON_SECRET` como **Sensitive**.
...
Use os valores do seu `.env.local` local (nunca versione esse arquivo):

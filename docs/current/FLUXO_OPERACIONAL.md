...
10. Encerra o ciclo somente quando todas as ações ativas estiverem concluídas,
   sem solicitações abertas e com aceite válido para a revisão atual. O
   encerramento não recalcula nem sobrescreve o FAMI.
11. Emite relatórios versionados do diagnóstico concluído.
12. Só reabre a validação enquanto não existir histórico de plano, supervisão ou
    exceção no processamento oficial, evitando recomendações e ações órfãs.
...
5. Analisa recomendações e cadastra uma ou mais ações por recomendação.
...
Formulário → Diagnóstico → Respostas e evidências → Validação → Resultado FAMI → Recomendações → Plano de ação ou exceção → Supervisão e aceite → Encerramento → Relatórios.

...
test.describe.serial("jornada canônica da plataforma", () => {
  ...
  test("admin solicita ajuste e o diagnóstico retorna para o respondente", async ({ page }) => {
    ...
    await page.getByRole("tab", { name: /Não se aplica/ }).click();
  ...
  test("respondente corrige arquivo, cria plano, admin valida e encerra", async ({ page }) => {
    ...
    await evidenceCard.locator('input[type="file"]').setInputFiles({
      name: "evidencia-e2e-corrigida.txt",
      mimeType: "text/plain",
      buffer: Buffer.from("evidência corrigida E2E"),
    });
    ...
    await page.getByRole("button", { name: "Emitir e baixar PDF" }).click();
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toBe("relatorio-orienta.pdf");
    await logout(page);
  });

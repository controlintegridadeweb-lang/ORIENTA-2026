...
--   • "Sim" com evidência obrigatória e sem documento bloqueia a consolidação
--     até decisão administrativa (`validation_unresolved_absent_proof`);
--   • `validate_without_proof` libera a finalização, pontua 0 de 2,0 (política
...
begin
  begin
    ...
    raise exception 'FALHOU(comprovação): aceitou Sim sem decisão administrativa';
  ...
  perform public.decide_response_without_proof(
    ...
    'Validação administrativa sem documento para o critério seed do FAMI.'
  );

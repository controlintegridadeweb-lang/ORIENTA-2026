...
> {
  ...
  if (cycle.state === "in_response") {
    const { data: editable, error: editableError } = await supabase.rpc(
      "is_cycle_question_collection_editable",
      {
        p_cycle_id: cycleId,
  ...
  if (transactionError) {
    ...
    if (hasDatabaseErrorCode(message, "response_revision_conflict")) {
      return {
        ok: false,
        status: 409,
        error: "Esta resposta foi alterada em outra aba ou por outro usuário. Recarregue o diagnóstico antes de salvar novamente.",
      };
    }

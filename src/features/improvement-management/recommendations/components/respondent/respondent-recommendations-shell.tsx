...
export function RespondentRecommendationsShell() {
  ...
  const totalForView = actionPlanView
    ? rows.filter((item) => item.hasPlan).length
    : summary.total;
  ...
  const listDescription = actionPlanView
    ? "Abra um plano para atualizar ações, responsáveis, início, final e progresso."
      ...
      <section className={`${layout.panelStack} pt-1`}>
        ...
        >
          <RespondentRecommendationFilters
            ...
            resultLabels={
              actionPlanView
                ? { singular: "plano de ação", plural: "planos de ação" }
                : undefined
            }
        ...
        >
          {filteredRows.length !== totalForView ? (
            <p className={typography.meta}>
              Exibindo{" "}
              <span className="font-medium tabular-nums text-slate-700">{filteredRows.length}</span>{" "}
              de <span className="tabular-nums text-slate-600">{totalForView}</span>{" "}
              {actionPlanView ? "planos" : "recomendações"} com os filtros atuais
            </p>
          ) : null}

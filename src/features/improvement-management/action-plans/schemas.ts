...
import { businessToday, isLocalDate } from "@/shared/datetime/business-date";
...
function refineActionDates(
  value: { startDate: string; dueDate: string; pastDueDateConfirmed?: boolean },
  ctx: z.RefinementCtx,
) {
  ...
  if (value.dueDate < businessToday() && !value.pastDueDateConfirmed) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["pastDueDateConfirmed"],
      message: "Confirme explicitamente o cadastro de um final anterior à data atual.",
    });
  }
}
...
export const createActionPlanSchema = z
  .object({
    ...
    pastDueDateConfirmed: z.boolean().optional().default(false),
  })
  .strict()

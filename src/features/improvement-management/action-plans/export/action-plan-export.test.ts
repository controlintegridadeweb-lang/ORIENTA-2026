...
describe("Excel analítico do plano de ação", () => {
  it("usa as 14 colunas canônicas, congela o cabeçalho e tipa datas e progresso", () => {
    ...
    const startCell = first[9] as { value: Date; format: string };
    const progressZero = first[12] as { value: number; format: string };
    const progressDone = second[12] as { value: number; format: string };
    ...
    const questionCell = first[4] as { wrap: boolean };
    const recommendationCell = first[5] as { wrap: boolean };
    const actionCell = first[7] as { wrap: boolean };

...
async function getActionPlanByRecommendation(
  ...
  if (!res.ok || !body.item) throw new Error(formatError(body));
  return body.item;
}
...
export function getAdminActionPlanByRecommendation(
  ...
  return getActionPlanByRecommendation(
    `/api/admin/action-plans/recommendations/${encodeURIComponent(recommendationId)}`,
  );
}
...
export function getRespondentActionPlanByRecommendation(
  ...
  return getActionPlanByRecommendation(
    `/api/respondent/action-plans/recommendations/${encodeURIComponent(recommendationId)}`,
  );
}
...
type SupervisionHistoryPageInput = HistoryPageInput & {

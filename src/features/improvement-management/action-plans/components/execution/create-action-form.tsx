  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createRespondentActionPlan({
        ...
        pastDueDateConfirmed: form.get("pastDueDateConfirmed") === "true",
      });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-base font-bold text-slate-900">Cadastrar nova ação</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl bg-white p-4 shadow-sm sm:p-5">
        ...
        {dueDate && dueDate < businessToday() ? (
          <label className={formSurface.messageWarning}>
            <span className="flex items-start gap-2">
              <input
                type="checkbox"
                name="pastDueDateConfirmed"
                value="true"
                className="mt-0.5"
                required
              />
              <span>Confirmo que o final é anterior à data atual e desejo mantê-lo.</span>
            </span>
          </label>
        ) : null}

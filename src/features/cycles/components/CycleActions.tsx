...
export function CycleActions({ cycle }: { cycle: CycleListItem }) {
  ...
  if (current === "completed") return <CompletedCycleActions controller={controller} />;
...
function CompletedCycleActions({ controller }: { controller: ReturnType<typeof useCycleActionsController> }) {
  ...
  return (
    <CycleActionSection
      ...
      description="Cria uma nova versão de processamento e preserva o resultado anterior no histórico."
      error={state.error}
      action={reopenTarget ? (
        <LoadingButton
          type="button"

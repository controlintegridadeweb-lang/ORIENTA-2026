    ...
    switch (currentStep) {
      case 1:
        return (
          ...
          >
            <div>
              <h3 className="text-base font-medium text-slate-900">{stepMeta.label}</h3>
              <p className="mt-1 text-sm text-slate-600">
                Nome de exibição visível para respondentes e relatórios. O formulário nasce como
                rascunho.
              </p>
            </div>
            <div className={formSurface.fieldGroup}>
              <label htmlFor="wizard-form-name" className={formSurface.label}>
                Nome do formulário
              </label>
              <input
                id="wizard-form-name"
        ...
        return formId ? (
          <div className="space-y-6">
            <div className="space-y-3">
              <StepHeading step={stepMeta} />
              <p className="text-sm text-slate-600">
                Cadastre cada pergunta e defina, no próprio card dela, a seção da biblioteca, a
                recomendação-base e a aplicabilidade por organização. A recomendação é gerada apenas
                quando a resposta for Não ou a evidência não for aprovada.
              </p>
            </div>
...
function StepHeading({ step }: { step: (typeof FORM_WIZARD_STEPS)[number] }) {
  return (
    <div>
      <h3 className="text-base font-medium text-slate-900">{step.label}</h3>
    </div>
  );
}

...
function buildSummary(openCount: number, awaitingActionCount: number): string {
  ...
  if (awaitingActionCount > 0) {
    return awaitingActionCount === openCount
      ? `${countLabel(awaitingActionCount, "recomendação", "recomendações")} sem plano de ação.`
      : `${countLabel(openCount, "recomendação em", "recomendações em")} acompanhamento · ${awaitingActionCount} sem plano.`;
  }

...
describe("prepareDetailedAnalysis", () => {
  it("ordena eixos e seções pela ordem oficial e mantém vínculos corretos", () => {
    ...
    expect(planning?.recommendations[0]?.actions[0]?.numberLabel).toBe("5.1.1.1-A1");
    expect(planning?.recommendations[0]?.actions[0]?.isOverdue).toBe(true);
    expect(planning?.recommendations[0]?.actions[0]?.movements.map((m) => m.id)).toEqual([
      "mov-1",
      "mov-2",
    ]);
    expect(planning?.recommendations[0]?.actions[0]?.movements[0]?.progressLabel).toBe(
      "0% -> 10%",
    );
    ...
    expect(planning?.recommendations[0]?.actions[1]?.movements).toHaveLength(0);
    ...
    expect(flat.numberLabels).toContain("5.1.1.1-A1");
    expect(flat.numberLabels).toContain("5.1.1.1-A2");
  });

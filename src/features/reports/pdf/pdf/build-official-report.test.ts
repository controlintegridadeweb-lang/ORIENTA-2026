...
describe("relatório oficial", () => {
  ...
  it("gera o PDF completo com hierarquia linear e metadados de integridade", async () => {
    ...
    expect(analysis.axes[0]?.sections[0]?.recommendations[0]?.actions[0]?.id).toBe("action-1");
    expect(analysis.axes[0]?.sections[0]?.recommendations[0]?.actions[0]?.movements[0]?.id).toBe(
      "mov-1",
    );

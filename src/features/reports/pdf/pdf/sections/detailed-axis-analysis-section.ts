...
function renderActions(
  ...
  let cur = doc.drawParagraph(cursor, "Plano de ação", {
    size: 9,
    bold: true,
  ...
  if (recommendation.actions.length === 0) {
    return doc.drawParagraph(cur, REPORT_EMPTY_RECOMMENDATION_ACTIONS, {
      size: 9,
      color: reportTheme.slate500,
      gap: 6,
    });
  ...
  cur = drawReportTable(
    ...
    cur,
    [
      { key: "action", header: "Ação", width: contentWidth() * 0.28 },
      { key: "responsible", header: "Responsável", width: contentWidth() * 0.18 },
      { key: "start", header: "Início", width: contentWidth() * 0.12 },
      { key: "end", header: "Final", width: contentWidth() * 0.12 },
      { key: "progress", header: "Progresso", width: contentWidth() * 0.12 },
...
function renderRecommendation(
  ...
  cur = renderActions(doc, cur, recommendation);

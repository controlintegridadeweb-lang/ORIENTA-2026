...
export function isSafeRespondentListPath(value: string): boolean {
  ...
  return (
    value === RESPONDENT_RECOMMENDATIONS_PATH ||
    value.startsWith(`${RESPONDENT_RECOMMENDATIONS_PATH}?`) ||
    value === RESPONDENT_ACTION_PLAN_LEGACY_PATH ||
    value.startsWith(`${RESPONDENT_ACTION_PLAN_LEGACY_PATH}?`)
  );
}

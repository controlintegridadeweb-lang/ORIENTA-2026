...
import { persistOfficialReport } from "@/features/reports/pdf/persist-official-report";
...
async function reportBytes(cycleId: string, actorUserId: string) {
  const client = createSupabaseServiceRoleClient();
  const data = await loadOfficialReportData({ cycleId });
  if (!data) {
  ...
  if (existing?.file_path) {
    ...
    emissionVersion = Number(existing.emission_version);
  } else {
    try {
      const emitted = await persistOfficialReport(client, { data, generatedBy: actorUserId });
      bytes = emitted.pdfBytes;
      ...
      if (hasDatabaseErrorCode(causeMessage, "cycle_not_completed")) {
        throw new PermanentReportFailure(
          "cycle_not_completed",
          "O relatório só pode ser emitido após encerrar a avaliação (situação Avaliação encerrada).",
          { cause: caught },
        );
      }
      throw caught instanceof Error ? caught : new Error("Falha ao emitir o relatório oficial.");
    }
  }

...
async function executeCycleAction(
  ...
  try {
    ...
    if (cycle.state === "completed") {
      return skip(cycle, "A avaliação já estava encerrada.");
    }
    if (cycle.state !== "validated") {
      return skip(cycle, "O ciclo precisa ter o diagnóstico validado antes do encerramento.");
    }
    const updated = await service.transition(cycle, "completed", actorUserId);
    return {
      cycleId: cycle.id,
      status: "succeeded",
      fromState: cycle.state,
      toState: updated.state,
      message: "Ciclo de acompanhamento encerrado sem recalcular o FAMI.",
    };
  } catch (error) {
    return {
      cycleId: cycle.id,
